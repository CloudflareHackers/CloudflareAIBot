...
var QueueBrokerObject = class extends MiniflareDurableObject {
  ...
  #flush = async () => {
    ...
    let retryAll = response.retryBatch.retry || response.outcome !== "ok", retryMessagesIds = new Set(response.retryMessages.map((msg) => msg.msgId)), failedMessages = 0, toRetry = [], toDeadLetterQueue = [];
    for (let message of batch)
      (retryAll || retryMessagesIds.has(message.id)) && (failedMessages++, message.incrementFailedAttempts() < maxAttempts ? (await this.logWithLevel(
        LogLevel.DEBUG,
    ...
    for (let message of messages) {
      let randomness = crypto.getRandomValues(new Uint8Array(16)), id = message.id ?? Buffer2.from(randomness).toString("hex"), timestamp = new Date(message.timestamp ?? this.timers.now()), body = deserialise(message);
